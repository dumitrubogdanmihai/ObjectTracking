...
#include <iostream>
#include <Windows.h> // for demo function
...
{
	cout << "CKalmanFilter class destructor was called" << endl;
}

void CKalmanFilter::getPredict(Point &objectPred, Rect &objectPredBox)
{
...
{
	if (object == Point(0,0) && objectBox == Rect(0, 0, 0, 0))
	{
		notFoundCount++;
...
void demo()
{
	...
	while (1)
	{
		...
		found ? kfilter.getPredict(predP, predR) : 0;
		...
		kfilter.printTrajectories(img.size());
